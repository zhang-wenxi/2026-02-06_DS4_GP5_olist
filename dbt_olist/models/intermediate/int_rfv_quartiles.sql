...
-- Stable anchor date from actual orders (not current_date)
max_date as (
    select max(cast(order_purchase_timestamp as date)) as snapshot_date
    from orders
),
...
calculated_metrics as (
    select
        ...
        frequency,
        monetary_value,
        -- FIXED: Use snapshot_date from max_date, not current_date()
        date_diff(
